...
# The original code is here: https://github.com/tbpaolini/Advent-of-Code/blob/master/2021/Day%2022/cubes2.py
#  The details: https://github.com/tbpaolini/Advent-of-Code/tree/master/2021/Day%2022
...
with open("../inputs/day22/input.txt") as file:
    for line in file:
...
for turn, new in instructions:
    ...
    for old in core:
        ...
        if old.min.x <= new.max.x <= old.max.x:  # Positive direction
            new_core += [
                Cuboid(Coordinate(new.max.x + 1, old.min.y, old.min.z), Coordinate(old.max.x, old.max.y, old.max.z))]
        ...
        if old.min.x <= new.min.x <= old.max.x:  # Negative direction
            new_core += [
                Cuboid(Coordinate(old.min.x, old.min.y, old.min.z), Coordinate(new.min.x - 1, old.max.y, old.max.z))]
        ...
        if old.min.y <= new.max.y <= old.max.y:  # Positive direction
            new_core += [
                Cuboid(Coordinate(old.min.x, new.max.y + 1, old.min.z), Coordinate(old.max.x, old.max.y, old.max.z))]
        ...
        if old.min.y <= new.min.y <= old.max.y:  # Negative direction
            new_core += [
                Cuboid(Coordinate(old.min.x, old.min.y, old.min.z), Coordinate(old.max.x, new.min.y - 1, old.max.z))]
        ...
        if old.min.z <= new.max.z <= old.max.z:  # Positive direction
            new_core += [
                Cuboid(Coordinate(old.min.x, old.min.y, new.max.z + 1), Coordinate(old.max.x, old.max.y, old.max.z))]
        ...
        if old.min.z <= new.min.z <= old.max.z:  # Negative direction
            new_core += [
                Cuboid(Coordinate(old.min.x, old.min.y, old.min.z), Coordinate(old.max.x, old.max.y, new.min.z - 1))]
            old = Cuboid(Coordinate(old.min.x, old.min.y, new.min.z), Coordinate(old.max.x, old.max.y, old.max.z))

    # Process the operation
    if turn == "on":
        new_core += [new]  # If the area is to be turned on, add the region of the instruction
    else:
        assert turn == "off"  # Ensure that there are no instructions besides "on" and "off"
